...
mapping each LSM level to its own reclaim unit yields a comparable reduction.
...
\label{sec:eval}
%===============================================================================
We answer three questions: (1) how much does placement reduce write
amplification at a representative operating point; (2) how does the benefit vary
with over-provisioning; and (3) how does it vary with workload separability.
...
Figure~\ref{fig:lsm} sweeps how many reclaim unit handles we use to hold the
levels, from one (all data in a single reclaim unit, which is equivalent to the
baseline) up to one per level. The baseline that issues no directive amplifies
the most; giving just the fast-churning top level its own reclaim unit already
removes most of the penalty, and separating every level roughly halves WAF, a
reduction on the same order as the two-class operating point. The curve has a
clear knee: the first split captures most of the benefit and further reclaim
units help less, which is direct guidance on how many a deployment actually
needs. This confirms the placement effect on a workload whose lifetimes come
from a real data structure, not a synthetic hot/cold model.
...
used to separate LSM levels. One handle reproduces the baseline; isolating the
top level captures most of the gain, and one handle per level roughly halves WAF.}
\label{fig:lsm}
\end{figure}
